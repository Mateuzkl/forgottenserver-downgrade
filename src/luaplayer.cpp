...
int luaPlayerGetSpectators(lua_State* L)
{
	Player* player = getUserdata<Player>(L, 1);
	if (!player) {
		pushBoolean(L, false);
...
int luaPlayerSetSpectators(lua_State* L)
{
	Player* player = getUserdata<Player>(L, 1);
	if (!player) {
		pushBoolean(L, false);
...
int luaPlayerSendCastChannelMessage(lua_State* L)
{
...
void LuaScriptInterface::registerPlayer()
{
	...
	registerMethod("Player", "getSpectators", luaPlayerGetSpectators);
	registerMethod("Player", "setSpectators", luaPlayerSetSpectators);
	registerMethod("Player", "sendCastChannelMessage", luaPlayerSendCastChannelMessage);
