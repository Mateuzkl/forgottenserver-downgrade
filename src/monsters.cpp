...
bool Monsters::deserializeSpell(const pugi::xml_node& node, spellBlock_t& sb, const std::string& description)
{
	...
	if ((attr = node.attribute("chance"))) {
		...
		if (chance > 100) {
			chance = 100;
			LOG_WARN(fmt::format("[Warning - Monsters::deserializeSpell] {} - Chance value out of bounds for spell: {}", description, name));
		}
		sb.chance = chance;
	} else if (boost::algorithm::to_lower_copy<std::string>(name) != "melee") {
		LOG_WARN(fmt::format("[Warning - Monsters::deserializeSpell] {} - Missing chance value on non-melee spell: {}", description, name));
	}
	...
	if (isScripted) {
		...
		if (!combatSpellPtr->loadScript(fmt::format("data/{}/scripts/{}", g_spells->getScriptBaseName(), scriptName))) {
			return false;
		}
		...
		if (tmpName == "melee") {
			...
			Condition* condition = Condition::createCondition(CONDITIONID_COMBAT, CONDITION_INVISIBLE, duration, 0).release();
			combat->setParam(COMBAT_PARAM_AGGRESSIVE, 0);
			combat->addCondition(condition);
			...
			Condition* condition =
			    Condition::createCondition(CONDITIONID_COMBAT, CONDITION_DRUNK, duration, drunkenness).release();
			combat->addCondition(condition);
			...
			LOG_ERROR(fmt::format("[Error - Monsters::deserializeSpell] - {} - Unknown spell name: {}", description, name));
			return false;
		}
				...
				if (caseInsensitiveEqual(value, "shooteffect")) {
					if ((attr = attributeNode.attribute("value"))) {
						...
						if (shoot != CONST_ANI_NONE) {
							...
							LOG_WARN(fmt::format("[Warning - Monsters::deserializeSpell] {} - Unknown shootEffect: {}", description, attr.as_string()));
						}
					}
					...
					if ((attr = attributeNode.attribute("value"))) {
						...
						if (effect != CONST_ME_NONE) {
							...
							LOG_WARN(fmt::format("[Warning - Monsters::deserializeSpell] {} - Unknown areaEffect: {}", description, attr.as_string()));
						}
					}
				} else {
					LOG_WARN(fmt::format("[Warning - Monsters::deserializeSpells] Effect type \"{}\" does not exist.", attr.as_string()));
				}
			}
		}
	}
...
MonsterType* Monsters::loadMonster(const std::string& file, const std::string& monsterName, bool reloading /*= false*/)
{
	...
	if (!monsterNode) {
		LOG_ERROR(fmt::format("[Error - Monsters::loadMonster] Missing monster node in: {}", file));
		return nullptr;
	}

	pugi::xml_attribute attr;
	if (!(attr = monsterNode.attribute("name"))) {
		LOG_ERROR(fmt::format("[Error - Monsters::loadMonster] Missing name in: {}", file));
		return nullptr;
	}
	...
	if ((attr = monsterNode.attribute("race"))) {
		...
		if (tmpStrValue == "venom" || tmpInt == 1) {
			...
			LOG_WARN(fmt::format("[Warning - Monsters::loadMonster] Unknown race type {}. {}", attr.as_string(), file));
		}
	}
	...
	if ((attr = monsterNode.attribute("raceId"))) {
		...
		if (raceId != 0) {
			...
			LOG_WARN(fmt::format("[Warning - Monsters::loadMonster] Invalid raceId 0. {}", file));
		}
	} else {
		mType->raceId = 0;
	}
	...
	if ((attr = monsterNode.attribute("script"))) {
		...
		if (scriptInterface->loadFile("data/monster/scripts/" + script) == 0) {
			...
			LOG_WARN(fmt::format("[Warning - Monsters::loadMonster] Can not load script: {}", script));
	...
	if ((node = monsterNode.child("health"))) {
		if ((attr = node.attribute("now"))) {
			...
			LOG_ERROR(fmt::format("[Error - Monsters::loadMonster] Missing health now. {}", file));
		}

		if ((attr = node.attribute("max"))) {
			mType->info.healthMax = pugi::cast<int32_t>(attr.value());
		} else {
			LOG_ERROR(fmt::format("[Error - Monsters::loadMonster] Missing health max. {}", file));
		}

		if (mType->info.health > mType->info.healthMax) {
			mType->info.health = mType->info.healthMax;
			LOG_WARN(fmt::format("[Warning - Monsters::loadMonster] Health now is greater than health max. {}", file));
		}
	}
	...
	if ((node = monsterNode.child("flags"))) {
		for (auto& flagNode : node.children()) {
			...
			if (caseInsensitiveEqual(attrName, "summonable")) {
			...
			} else if (caseInsensitiveEqual(attrName, "isrewardboss")) {
				...
				if (staticAttack > 100) {
					LOG_WARN(fmt::format("[Warning - Monsters::loadMonster] staticattack greater than 100. {}", file));
					staticAttack = 100;
				}
				...
				if (targetDistance < 1) {
					targetDistance = 1;
					LOG_WARN(fmt::format("[Warning - Monsters::loadMonster] targetdistance less than 1. {}", file));
				}
				...
				LOG_WARN(fmt::format("[Warning - Monsters::loadMonster] Unknown flag attribute: {}. {}", attrName, file));
			}
		}
	...
	if (mType->info.manaCost == 0 && (mType->info.isSummonable || mType->info.isConvinceable)) {
		LOG_WARN(fmt::format("[Warning - Monsters::loadMonster] manaCost missing or zero on monster with summonable and/or convinceable flags: {}", file));
	}
	...
	if ((node = monsterNode.child("targetchange"))) {
		if ((attr = node.attribute("speed")) || (attr = node.attribute("interval"))) {
			...
			LOG_WARN(fmt::format("[Warning - Monsters::loadMonster] Missing targetchange speed. {}", file));
		}
		...
		if ((attr = node.attribute("chance"))) {
			...
			if (chance > 100) {
				chance = 100;
				LOG_WARN(fmt::format("[Warning - Monsters::loadMonster] targetchange chance value out of bounds. {}", file));
			}
			mType->info.changeTargetChance = chance;
		} else {
			LOG_WARN(fmt::format("[Warning - Monsters::loadMonster] Missing targetchange chance. {}", file));
		}
	}
	...
	if ((node = monsterNode.child("look"))) {
		if ((attr = node.attribute("type"))) {
			...
			LOG_WARN(fmt::format("[Warning - Monsters::loadMonster] Missing look type/typeex. {}", file));
		}
	...
	if ((node = monsterNode.child("attacks"))) {
		for (auto& attackNode : node.children()) {
			...
			if (deserializeSpell(attackNode, sb, monsterName)) {
				...
				LOG_WARN(fmt::format("[Warning - Monsters::loadMonster] Cant load spell. {}", file));
			}
		}
	}
		...
		for (auto& immunityNode : node.children()) {
			if ((attr = immunityNode.attribute("name"))) {
				...
				if (tmpStrValue == "physical") {
					...
					LOG_WARN(fmt::format("[Warning - Monsters::loadMonster] Unknown immunity name {}", attr.as_string()));
				}
				...
				LOG_WARN(fmt::format("[Warning - Monsters::loadMonster] Unknown immunity. {}", file));
			}
		}
	}
	...
	if ((node = monsterNode.child("voices"))) {
		if ((attr = node.attribute("speed")) || (attr = node.attribute("interval"))) {
			...
			LOG_WARN(fmt::format("[Warning - Monsters::loadMonster] Missing voices speed. {}", file));
		}
		...
		if ((attr = node.attribute("chance"))) {
			...
			if (chance > 100) {
				chance = 100;
				LOG_WARN(fmt::format("[Warning - Monsters::loadMonster] yell chance value out of bounds. {}", file));
			}
			mType->info.yellChance = chance;
		} else {
			LOG_WARN(fmt::format("[Warning - Monsters::loadMonster] Missing voices chance. {}", file));
		}
		...
		for (auto& voiceNode : node.children()) {
			...
			if ((attr = voiceNode.attribute("sentence"))) {
				...
				LOG_WARN(fmt::format("[Warning - Monsters::loadMonster] Missing voice sentence. {}", file));
			}
	...
	if ((node = monsterNode.child("loot"))) {
		for (auto& lootNode : node.children()) {
			...
			if (loadLootItem(lootNode, lootBlock)) {
				...
				LOG_WARN(fmt::format("[Warning - Monsters::loadMonster] Cant load loot. {}", file));
			}
		}
	}
		...
		for (auto& elementNode : node.children()) {
			if ((attr = elementNode.attribute("physicalPercent"))) {
				...
				if (mType->info.damageImmunities & COMBAT_PHYSICALDAMAGE) {
					LOG_WARN(fmt::format("[Warning - Monsters::loadMonster] Same element \"physical\" on immunity and element tags. {}", file));
				}
				...
				if (mType->info.damageImmunities & COMBAT_ICEDAMAGE) {
					LOG_WARN(fmt::format("[Warning - Monsters::loadMonster] Same element \"ice\" on immunity and element tags. {}", file));
				}
				...
				if (mType->info.damageImmunities & COMBAT_EARTHDAMAGE) {
					LOG_WARN(fmt::format("[Warning - Monsters::loadMonster] Same element \"earth\" on immunity and element tags. {}", file));
				}
				...
				if (mType->info.damageImmunities & COMBAT_FIREDAMAGE) {
					LOG_WARN(fmt::format("[Warning - Monsters::loadMonster] Same element \"fire\" on immunity and element tags. {}", file));
				}
				...
				if (mType->info.damageImmunities & COMBAT_ENERGYDAMAGE) {
					LOG_WARN(fmt::format("[Warning - Monsters::loadMonster] Same element \"energy\" on immunity and element tags. {}", file));
				}
				...
				if (mType->info.damageImmunities & COMBAT_HOLYDAMAGE) {
					LOG_WARN(fmt::format("[Warning - Monsters::loadMonster] Same element \"holy\" on immunity and element tags. {}", file));
				}
				...
				if (mType->info.damageImmunities & COMBAT_DEATHDAMAGE) {
					LOG_WARN(fmt::format("[Warning - Monsters::loadMonster] Same element \"death\" on immunity and element tags. {}", file));
				}
				...
				if (mType->info.damageImmunities & COMBAT_DROWNDAMAGE) {
					LOG_WARN(fmt::format("[Warning - Monsters::loadMonster] Same element \"drown\" on immunity and element tags. {}", file));
				}
				...
				if (mType->info.damageImmunities & COMBAT_LIFEDRAIN) {
					LOG_WARN(fmt::format("[Warning - Monsters::loadMonster] Same element \"lifedrain\" on immunity and element tags. {}", file));
				}
				...
				if (mType->info.damageImmunities & COMBAT_MANADRAIN) {
					LOG_WARN(fmt::format("[Warning - Monsters::loadMonster] Same element \"manadrain\" on immunity and element tags. {}", file));
				}
			} else {
				LOG_WARN(fmt::format("[Warning - Monsters::loadMonster] Unknown element percent. {}", file));
			}
		}
	}
	...
	if ((node = monsterNode.child("summons"))) {
		if ((attr = node.attribute("maxSummons"))) {
			...
			LOG_WARN(fmt::format("[Warning - Monsters::loadMonster] Missing summons maxSummons. {}", file));
		}
		...
		for (auto& summonNode : node.children()) {
			...
			if ((attr = summonNode.attribute("chance"))) {
				...
				if (chance > 100) {
					chance = 100;
					LOG_WARN(fmt::format("[Warning - Monsters::loadMonster] Summon chance value out of bounds. {}", file));
				}
			}
					...
					if (caseInsensitiveEqual(value, "mastereffect")) {
						if ((attr = attributeNode.attribute("value"))) {
							...
							if (masterEffect == CONST_ME_NONE) {
								LOG_WARN(fmt::format("[Warning - Monsters::loadMonster] Summon master effect - Unknown masterEffect: {}", attr.as_string()));
							}
						}
						...
						if ((attr = attributeNode.attribute("value"))) {
							...
							if (effect == CONST_ME_NONE) {
								...
								LOG_WARN(fmt::format("[Warning - Monsters::loadMonster] Summon effect - Unknown effect: {}", attr.as_string()));
							}
						}
					} else {
						LOG_WARN(fmt::format("[Warning - Monsters::loadMonster] Summon effect type \"{}\" does not exist.", attr.as_string()));
					}
				}
			}
			...
			if ((attr = summonNode.attribute("name"))) {
				...
				LOG_WARN(fmt::format("[Warning - Monsters::loadMonster] Missing summon name. {}", file));
			}
		}
	}
	...
	if ((node = monsterNode.child("script"))) {
		for (auto& eventNode : node.children()) {
			if ((attr = eventNode.attribute("name"))) {
				...
				LOG_WARN(fmt::format("[Warning - Monsters::loadMonster] Missing name for script event. {}", file));
			}
		}
	}
...
bool Monsters::loadLootItem(const pugi::xml_node& node, LootBlock& lootBlock)
{
	...
	if ((attr = node.attribute("id"))) {
		...
		if (it.name.empty()) {
			LOG_WARN(fmt::format("[Warning - Monsters::loadMonster] Unknown loot item id \"{}\".", id));
			return false;
		}
		...
		if (ids.first == Item::items.nameToItems.cend()) {
			LOG_WARN(fmt::format("[Warning - Monsters::loadMonster] Unknown loot item \"{}\".", name));
			return false;
		}
		...
		if (std::next(ids.first) != ids.second) {
			LOG_WARN(fmt::format("[Warning - Monsters::loadMonster] Non-unique loot item \"{}\".", name));
			return false;
		}
	...
	if ((attr = node.attribute("chance")) || (attr = node.attribute("chance1"))) {
		...
		if (lootChance > static_cast<int32_t>(MAX_LOOTCHANCE)) {
			LOG_WARN(fmt::format("[Warning - Monsters::loadMonster] Invalid \"chance\" {} used for loot, the max is {}.", lootChance, MAX_LOOTCHANCE));
		}
