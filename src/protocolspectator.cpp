...
void ProtocolSpectator::spectatorSay(ProtocolGame_ptr spectator, const std::string& text)
{
	...
	std::string command = std::string(t[0]);
	toLowerCaseString(command);
        if (command == "list" || command == "show" || command == "spectators")
        {
            std::stringstream s;
        ...
        else if (t[0] == "name" || t[0] == "nick")
        {
            ...
            sendCastMessage("", spectator->getSpectatorName() + " has changed name to " + name + ".", TALKTYPE_CHANNEL_O);
